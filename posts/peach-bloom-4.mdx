...
Lighthouse의 Performance 성능을 개선하여 성능 57% 개선한 내용에 대해 다룹니다.
...
priority는 우선순위라는 뜻으로, 이 속성을 설정하면 브라우저가 미리 렌더링을 합니다. <br/>
그래서 랜딩 페이지에서 제일 처음 보이는 이미지에는 priority을 지정하는게 성능면에서 당연히 유리합니다.

사실 처음에는 모든 Image에 priority를 설정하면 LCP 성능이 개선되는줄 알았습니다.
하지만 모든 Image에 priority를 설정하면, 웹 브라우저는 모든 이미지를 동시에 불러오려고 시도하고, 이는 네트워크에 부담을 주기에 LCP를 느리게 만들 수 있습니다.
또한, 중요하지 않은 이미지가 먼저 로드되어 중요한 이미지의 로드를 지연시키는 이슈가 발생할 수도 있습니다.

priority 속성은 대표적으로 메인 화면에서 유용하게 사용했습니다. 화면 로드 시 처음에 보이는 이미지의 개수만 priority 속성을 적용
시키기 위해 `priorityIndices` prop을 `CarouselSwiper`컴포넌트에 전달해주며 type 별로 초기 로드되는 이미지의 개수를 다르게 설정했습니다.
...
priority를 적절하게 설정하여 메인 화면에서 BEST 상품은 1개, 나머지는 각 3개씩 총 7개의 이미지가 미리 렌더링 됩니다.
...
로컬에서 사용하는 이미지인 `Symbol`과 `Logo`에 blur 처리가 되어 CLS를 개선했습니다.
...
- **swap 속성**
  ...
  따라서 화면이 비어있는 시간이 줄어들어 FP(First Content Paint) 시간을 단축할 수 있습니다.
...
[Next.js의 가이드](https://nextjs.org/docs/pages/building-your-application/optimizing/fonts#local-fonts)에 따라 최적화를 진행했습니다.<br/>
next/font/local의 로컬 폰트를 로드하고 **swap 설정**을 해줬고, layout의 **html에 적용**했습니다.
...
폰트가 google이 아닌 next에서 로드되는 것을 확인할 수 있으며 CLS 점수도 0점으로 최적화할 수 있었습니다.
...
Lighthouse를 통해 이미지와 폰트 등 다양한 리소스의 최적화 방법과 번들 사이즈 측정 등 다양한 방식으로 개선해볼 수 있었습니다.

✨ **최적화 이전** Lighthouse의 Performance 점수는 **67점**입니다.
...
✨ **최적화 이후** Lighthouse의 Performance 점수는 **97점**으로, 총 **57% 향상에 성공**했습니다.<br/>
특히 LCP와 Speed Index가 가장 많이 개선되었고, 개선 전 보다 실제로 웹사이트의 UX가 유연함을 느꼈습니다.
...
부끄럽지만 현업에서도 이전까지의 프로젝트에서도 이렇게 성능 개선에 집중했던 경험이 없었습니다. 항상 기능 구현에 급급하게 개발하던 제가
성능에 집중하며 테스트를 한다니 감개무량합니다.

물론 아직 더 개선해야할 점이 많이 남아있으며, 추후 First Load JS에 집중해 개선 예정이고 댓글과 스켈레톤 UI, 결제 모듈 연결 등
추가 기능도 개발 예정입니다.

#### ✨ 총 정리

- 성능 57% 개선
- bundle-analyzer를 사용해 번들 사이즈 측정 후 Client 번들 파일을 위주로 사이즈가 크고 투자대비 효율이 좋은 순서로 리팩토링 진행
- Image 컴포넌트의 srcSet을 필요에 맞게 변경 및 필요한 이미지 개수만 priority를 적절히 설정하여 이미지 최적화
- Cloudinary에 이미지를 업로드하고 관리하며, 포멧 최적화 및 리사이징, 크롭핑을 이용해 이미지들의 평균 사이즈 80% 이상 대폭 감소 (캐러셀 이미지 45KB → 8KB)
- Tree Shaking을 적용해 사용하지 않는 코드 제거 및 번들 사이즈 최소화
- Next.js의 Dynamic import를 활용해 코드 분할을 적용해 번들 파일 사이즈 감소 및 초기 로딩 속도 향상

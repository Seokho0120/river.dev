    ...
    <section className='mb-10'>
      ...
      {test.map(({ title, job, date, lists, href }, idx) => (
        <div className='flex mb-6' key={idx}>
          <div className='w-1/3'>
            <p className='text-xl font-bold mb-1 cursor-pointer hover:underline decoration-point underline-offset-2'>
              <Link href={href}>{title}</Link>
            </p>
            <p className='text-sm italic text-neutral-600'>{job}</p>
            <p className='text-sm italic text-neutral-600'>{date}</p>
          </div>
          <ul className='w-2/3'>
            {lists.map((list, idx) => (
              <li
              ...
              >
                <circle className='w-1 h-1 bg-point rounded-full absolute left-0 top-[0.6rem] inline-block' />
                {list}
              </li>
            ))}
          </ul>
